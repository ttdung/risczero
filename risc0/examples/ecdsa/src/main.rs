...
fn main() {
    ...
    let message1 = b"This is a message that will be signed, and verified within the zkVM1";

    // Run signature verified in the zkVM guest and get the resulting receipt.
    let receipt = prove_ecdsa_verification(signing_key.verifying_key(), message1, &signature);

    // Verify the receipt and then access the journal.
    println!("ECDSA_VERIFY_ID:{:?}", ECDSA_VERIFY_ID);

    let rs = receipt.verify(ECDSA_VERIFY_ID).unwrap();
    let (receipt_verifying_key, receipt_message): (EncodedPoint, Vec<u8>) =
        receipt.journal.decode().unwrap();

    println!(
        "Verified the signature over message {:?} with key {} result {:?}",
        std::str::from_utf8(&receipt_message[..]).unwrap(),
        receipt_verifying_key, rs,
    );
}
